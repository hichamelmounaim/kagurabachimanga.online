...
const STORAGE_KEY = 'kagurabachi_read_chapters';
      ...
      <div className="bg-white dark:bg-[#0f0f0f] border-b border-gray-100 dark:border-white/5 py-10 px-4">
        <div className="max-w-4xl mx-auto">
          ...
          <p className="text-gray-600 dark:text-gray-400 text-sm leading-relaxed mb-4">
            Chihiro Rokuhira, son of legendary swordsmith Kunishige Rokuhira, seeks revenge against the Hishaku sorcerers who murdered his father and stole the family's enchanted blades. Armed with the powerful blade Enten, Chihiro embarks on a brutal journey through a world of sorcery and blade combat.
          </p>
          <div className="flex flex-wrap gap-6 text-xs text-gray-500 dark:text-gray-500">
            <span><strong className="text-gray-700 dark:text-gray-300">Author:</strong> Takeru Hokazono</span>
            <span><strong className="text-gray-700 dark:text-gray-300">Publisher:</strong> Shueisha / Weekly Shōnen Jump</span>
            <span><strong className="text-gray-700 dark:text-gray-300">Status:</strong> Ongoing</span>
            <span><strong className="text-gray-700 dark:text-gray-300">Started:</strong> September 2023</span>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white dark:bg-[#1a1a1a] p-4 rounded-xl shadow-xl shadow-black/10 border border-gray-100 dark:border-white/5 flex flex-col md:flex-row items-center justify-between gap-4 mb-10 backdrop-blur-md">
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-lg hover:bg-gray-100 dark:hover:bg-white/10 dark:text-white transition-colors font-medium min-w-[110px] justify-center"
            >
              <Filter size={16} />
        ...
        {readChapters.size > 0 && (
          <div className="flex items-center gap-3 mb-6 px-1">
            <div className="flex-1 h-1.5 bg-gray-200 dark:bg-white/10 rounded-full overflow-hidden">
              <div className="h-full bg-green-500 transition-all duration-500 rounded-full" style={{ width: `${Math.round((readChapters.size / chapters.length) * 100)}%` }} />
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              ...
              return (
                ...
                >
                  ...
                  <div className="relative z-10">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span className="text-[10px] font-mono text-gray-300 dark:text-gray-600 bg-gray-100 dark:bg-white/5 px-2 py-0.5 rounded">
                        {new Date(chapter.releaseDate).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}
                      </span>
                    </div>
                    <h3 className={`text-base font-bold transition-colors line-clamp-1 pr-4 ${isRead ? 'text-gray-500 dark:text-gray-400 group-hover:text-bb-blue' : 'text-gray-900 dark:text-white group-hover:text-bb-blue'}`}>{chapter.title}</h3>
                  </div>
                  <div className="relative z-10 flex items-center justify-between mt-auto pt-2">
                    ...
                    <button
                      onClick={(e) => toggleRead(e, chapter.number)}
                      className={`w-7 h-7 rounded-full flex items-center justify-center transition-all border ${isRead ? 'bg-green-500 border-green-500 text-white hover:bg-red-500 hover:border-red-500' : 'border-gray-300 dark:border-white/20 text-gray-400 hover:border-green-500 hover:text-green-500'}`}
            ...
            {filteredChapters.length === 0 && (
              <div className="col-span-full py-20 text-center">
                <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-gray-100 dark:bg-white/5 mb-4"><Search size={32} className="text-gray-400" /></div>
